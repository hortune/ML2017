...
class Generative(object):
    def guassian_mean_sigma(self,c1):
        mean = c1.sum(axis=0)/c1.shape[0]
        x_minus_mean = c1-mean
        x_minus_mean = x_minus_mean.sum(axis=0)/c1.shape[0]
    ...
    def guassian_model(self,c1,c2):
        ...
        print(np.linalg.det(true_sigma))
...
def activate(self,x):
        ...
        ans = (true_x.dot(self.true_inverse)*true_x).sum(axis=1)
        print(ans.shape)
